...
# Separar la columna 'UsuarioMensaje' en 'Usuario' y 'Mensaje'
data[['Usuario', 'Mensaje']] = data['UsuarioMensaje'].str.split(':', n=1, expand=True)

# Eliminar columnas que no son necesarias
data = data[['Fecha', 'Hora', 'Usuario', 'Mensaje']]
...
try:
    ...
    with connection.cursor() as cursor:
        ...
        for _, row in data.iterrows():
            ...
            WHERE fecha = %s AND hora = %s AND usuario = %s AND mensaje = %s;
            """
            
            # Ejecutar la consulta de verificación
            cursor.execute(sql_check, (row['Fecha'].strip(), row['Hora'].strip(), row['Usuario'].strip(), row['Mensaje'].strip()))
            result = cursor.fetchone()
            
            # Si no existe el registro, proceder con la inserción
            if result[0] == 0:  # Si COUNT(*) = 0, no existe duplicado
                sql_insert = """
                INSERT INTO mensajes_whatsapp (fecha, hora, usuario, mensaje)
                VALUES (%s, %s, %s, %s);
                """
                cursor.execute(sql_insert, (row['Fecha'].strip(), row['Hora'].strip(), row['Usuario'].strip(), row['Mensaje'].strip()))
                print(f"Registro insertado: {row['Fecha']} {row['Hora']} {row['Usuario']} - {row['Mensaje']}")
            else:
                print(f"Registro duplicado encontrado y omitido: {row['Fecha']} {row['Hora']} {row['Usuario']} - {row['Mensaje']}")
